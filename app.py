...
tab_scan,tab_team=st.tabs(["📊 Scanner","🔎 Buscar por time"])
...
with tab_scan:
    ...
    live=[e for e in events if e["category"]=="AO VIVO"]; upcoming=[e for e in events if e["category"]=="PRÓXIMO"]
    c1,c2,c3=st.columns(3); c1.metric("🟢 Ao vivo",len(live)); c2.metric("🔵 Próximos",len(upcoming)); c3.metric("Total analisável",len(events))
    if not events:
        st.warning("Nenhum jogo analisável encontrado. Verifique a data e as chaves opcionais."); st.stop()
    ...
    progress.empty(); df=pd.DataFrame(rows)
    ...
    def render_match_card(row, idx, live_mode=False):
        ...
        with st.container(border=True):
            ...
            with c2:
                st.metric("Melhor opção", row["Sugestão"], row["Prob."])
            with c3:
                st.metric("Confiança", row["Confiança"])
            b1,b2,b3,b4,b5=st.columns(5)
            b1.metric("Casa",row["Casa"])
            b2.metric("Empate",row["Empate"])
            b3.metric("Fora",row["Fora"])
            b4.metric("BTTS",row["BTTS"])
            b5.metric("O2.5",row["O2.5"])
            if live_mode:
                if ev.get("minute") is not None:
                    st.caption(f"⏱️ Minuto: {ev.get('minute')}  |  Placar: {row['Placar']}")
                live_stats=[]
                for key,label in [("home_corners","Esc. casa"),("away_corners","Esc. fora"),("home_shots","Chutes casa"),("away_shots","Chutes fora"),("home_dangerous","Ataques perigosos casa"),("away_dangerous","Ataques perigosos fora")]:
                    if ev.get(key) is not None:
                        live_stats.append(f"{label}: {ev[key]}")
                if live_stats:
                    st.caption(" • ".join(live_stats))
            with st.expander("📊 Ver análise completa",expanded=False):
                ...
                with x2:
                    ...
                    for nome,prob in suggestions(pred):
                        st.write(f"• {nome}: {pct(prob)}")
                h=pred.get("h2h") or {}
                if h.get("games"):
    ...
    if live_df.empty:
        ...
        st.caption(f"{len(live_df)} partida(s) ao vivo — clique em **Ver análise completa** para abrir os detalhes.")
        for idx,(_,r) in enumerate(live_df.iterrows()):
            render_match_card(r,idx,live_mode=True)
    ...
    if up_df.empty:
        ...
        st.caption(f"{len(up_df)} próximo(s) jogo(s)")
        # Tabela compacta para consulta rápida; cada linha continua disponível no painel de detalhes abaixo.
    ...
    st.download_button("⬇️ Baixar CSV",csv,"football_scanner_v6.csv","text/csv")
    st.subheader("🎯 Melhores oportunidades")
    for _,r in df.sort_values("Prob.",ascending=False).head(10).iterrows():
        st.markdown(f"**{r['Status']} — {r['Jogo']}** — {r['Sugestão']} ({r['Prob.']}) | Confiança {r['Confiança']} | H2H: {r['H2H']}")
