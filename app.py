...
FD5_TOKEN = os.getenv("FIVEDOLLAR_FOOTBALL_API_KEY", os.getenv("FIVEDOLLAR_FOOTBALLAPI_KEY", ""))
...
def is_live_status(status: Any) -> bool:
    ...
    if compact in {"live", "inplay", "playing", "1h", "2h", "ht", "halftime", "firsthalf", "secondhalf", "extratime", "et", "penalties", "penalty", "pen"}:
        return True
    if any(x in compact for x in ("secondhalf", "firsthalf", "inplay", "extratime")):
        return True
    if compact.startswith(("1h", "2h", "1st", "2nd")):
        return True
    return False
...
def parse_kickoff(e: Dict[str, Any]) -> Optional[datetime]:
    ...
    if d and t:
        try:
            return datetime.fromisoformat(f"{d}T{t[:8]}").replace(tzinfo=timezone.utc).astimezone(BRT)
        except Exception:
            pass
    return None
...
def classify_event(e: Dict[str, Any]) -> str:
    ...
    compact = re.sub(r"[^a-z0-9]", "", s)
    if s in FINAL_STATUSES or any(x in compact for x in ("finished", "fulltime", "completed", "ended")):
        return "FINALIZADO"
    if is_live_status(s):
        return "AO VIVO"
    has_score = e.get("home_score") is not None or e.get("away_score") is not None
    if has_score and s not in PRE_STATUSES and s not in FINAL_STATUSES:
        return "AO VIVO"
    ko = parse_kickoff(e)
    if ko:
        now = datetime.now(timezone.utc).astimezone(BRT)
        if now - timedelta(hours=3, minutes=30) <= ko <= now and (s not in PRE_STATUSES or has_score):
            return "AO VIVO"
    return "PRÓXIMO"
...
def request_openfoot(params=None):
    """OpenFoot public preview first; if a configured key fails, retry without auth."""
    base_headers = {"Accept": "application/json"}
    if OPENFOOT_TOKEN:
        h = dict(base_headers)
        h["Authorization"] = f"Bearer {OPENFOOT_TOKEN}"
        data = request_json(f"{OPENFOOT_BASE}/matches", headers=h, params=params or {})
        if data is not None:
            return data
    return request_json(f"{OPENFOOT_BASE}/matches", headers=base_headers, params=params or {})
...
def openfoot_matches(day):
    # Date feed: works with public preview and with a Developer key.
    data = request_openfoot({"date": day}) or {}
    rows = data.get("data") or []

    # Explicit live query prevents a second-half match from disappearing when
    # the date feed is incomplete or changes its status representation.
    live_data = request_openfoot({"date": day, "status": "live"}) or {}
    live_rows = live_data.get("data") or []

    seen = {str(x.get("id")) for x in rows if x.get("id") is not None}
    for x in live_rows:
...
def fd5_day(day):
    ...
    headers = {"Authorization": f"Bearer {FD5_TOKEN}", "Accept": "application/json"}

    rows = []
    # Normal day window; native API allows up to 24h.
    for page in (1, 2, 3, 4, 5):
        data = request_json(
            f"{FD5_BASE}/fixtures",
            headers=headers,
            params={"start_time": start, "end_time": end, "per_page": 100, "page": page}
        ) or {}
        batch = data.get("data") or []
        rows.extend(batch)
        pag = data.get("pagination") or {}
        if not pag.get("has_more") or not batch:
            break

    # Explicit live feed: returns every in-play match regardless of date.
    live_data = request_json(
        f"{FD5_BASE}/fixtures",
        headers=headers,
        params={"status": "live", "per_page": 500}
    ) or {}
    rows.extend(live_data.get("data") or [])

    unique = {}
    for x in rows:
        unique[str(x.get("id"))] = x
...
def load_global_events(target, days_ahead=1):
    ...
    unique={}; priority={"OpenFootAPI":5,"5DollarFootballAPI":4,"football-data.org":2,"TheSportsDB":1}
    for e in events:
...
# Atualização automática do painel ao vivo a cada 300 segundos.
# Atualização automática a cada 300 segundos sem depender de pacote externo.
# Usa o mecanismo nativo do navegador para recarregar a página.
st.markdown(
...
with st.sidebar:
    ...
    st.write("🌐 OpenFootAPI:","chave configurada + fallback público" if OPENFOOT_TOKEN else "modo público")
    st.caption("OpenFoot público: até 5 partidas por chamada; com chave, cobertura maior.")
    if not FD5_TOKEN:
        st.warning("5DollarFootballAPI: configure FIVEDOLLAR_FOOTBALL_API_KEY nos Secrets para ativar.")
...
src_counts = {}
for _e in events:
    src_counts[_e["source"]] = src_counts.get(_e["source"], 0) + 1
with st.expander("🔧 Diagnóstico das fontes", expanded=False):
    st.write(src_counts or {"nenhuma": 0})
    st.caption("Se OpenFoot aparecer como 0, a API pública/chave não retornou partidas para o período consultado. Se 5Dollar aparecer como 0, configure a chave nos Secrets.")
