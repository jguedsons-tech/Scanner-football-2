...
def suggestions(pred,min_prob=.65):
    ...
    return sorted([(m,v) for m,v in c if v>=min_prob],key=lambda x:x[1],reverse=True)[:5]
...
st.caption("4 fontes + últimos 20 jogos + H2H | jogos ao vivo permanecem no scanner")

with st.sidebar:
    st.header("⚙️ Configuração")
    target=st.date_input("Data inicial",value=date.today())
    days=st.slider("Dias para varrer",0,3,1)
    min_prob=st.slider("Probabilidade mínima",0.65,0.95,0.65,0.01)
...
with tab_scan:
    ...
    c1,c2,c3,c4=st.columns(4); c1.metric("🟢 Ao vivo",len(live)); c2.metric("🔵 Próximos",len(upcoming)); c3.metric("Total analisável",len(events)); c4.metric("Filtro",f"≥ {min_prob:.0%}")
    if not events:
    ...
    for i,e in enumerate(events):
        pred=build_prediction(e,form_n); sugs=suggestions(pred,min_prob); best=sugs[0] if sugs else ("Sem sugestão ≥ filtro",0)
    ...
    st.dataframe(live_df[display],use_container_width=True,hide_index=True) if not live_df.empty else st.info("Nenhum jogo ao vivo neste momento.")
    st.subheader("🔵 PRÓXIMOS JOGOS")
    up_df=df[df["Status"]=="PRÓXIMO"]
    st.dataframe(up_df[display],use_container_width=True,hide_index=True) if not up_df.empty else st.info("Nenhum próximo jogo encontrado.")
    csv=df[display].to_csv(index=False).encode("utf-8-sig")
    st.download_button("⬇️ Baixar CSV",csv,"football_scanner_v5.csv","text/csv")
